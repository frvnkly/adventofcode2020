...
def parse_passports(_input: str) -> List[Passport]:
    passports = list()
...
def count_valid_passports(passports: List[Passport]) -> Tuple[int,int]:
    num_complete = 0
    num_valid = 0
    for passport in passports:
        ...
        if passport.get('hgt') is None:
            ...
            if not re.match('((1[5-8][0-9]|19[0-3])cm|(59|6[0-9]|7[0-6])in)', passport['hgt']):
                valid = False
    ...
    return (num_complete, num_valid)

if __name__ == '__main__':
    passports = parse_passports('input.txt')
    complete_passports, valid_passports = count_valid_passports(passports)

    print('Part 1:', complete_passports)
